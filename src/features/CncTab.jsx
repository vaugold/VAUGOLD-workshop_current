...
export const CncTab = ({
  ...
  orders = [], repairs = [], updateOrder, editOrder,
  ...
  // ИСПРАВЛЕНО 2026-06-29: ?? [] и ?? 0 — защита от возможного undefined в HMR/race-condition.
  // Не должно быть undefined в обычной работе, но JSX-выражение {fmt(totalKirill)}
  // выкидывало ReferenceError в консоли у пользователя — теперь не выкинет.
  const totalKirill = (vauOrders || []).reduce((s, o) => {
    const ks = (o.stages || []).filter(st => st && st.employee === MASTERS.KIRILL)
      .reduce((s2, st) => s2 + (parseFloat(st.cost) || 0), 0);
    return s + (o.location === "L24" ? ks * 0.8 : ks);
  }, 0);

  // ДОБАВЛЕНО 2026-06-28: агрегация гравировок по ремонтам (отдельная карточка в CNC)
  const engravingList = useMemo(() => {
    return (repairs || [])
      .filter(r => r.engraving && r.engraving.enabled)
      .map(r => ({
        ...r,
        _engravingPrice: parseFloat(r.engraving.price) || 0,
        _engravingKirill: parseFloat(r.engraving.kirillCost) || 0,
        _engravingOutsource: parseFloat(r.engraving.outsourceCost) || 0,
        _engravingMarkup: Math.max(0, (parseFloat(r.engraving.price) || 0) - (parseFloat(r.engraving.kirillCost) || 0) - (parseFloat(r.engraving.outsourceCost) || 0))
      }))
      .sort((a, b) => (b.orderDate || "").localeCompare(a.orderDate || ""));
  }, [repairs]);

  const engravingTotals = (engravingList || []).reduce((acc, r) => ({
    count: (acc.count || 0) + 1,
    price: (acc.price || 0) + (r._engravingPrice || 0),
    kirill: (acc.kirill || 0) + (r._engravingKirill || 0),
    outsource: (acc.outsource || 0) + (r._engravingOutsource || 0),
    markup: (acc.markup || 0) + (r._engravingMarkup || 0)
  }), { count: 0, price: 0, kirill: 0, outsource: 0, markup: 0 });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        {/* ДОБАВЛЕНО 2026-06-28: отдельная вкладка «Гравировка» */}
        <button className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${view === "engraving" ? "bg-purple-600 text-white shadow-md" : "bg-white text-purple-600 border border-purple-200 hover:bg-purple-50"}`} onClick={() => setView("engraving")}>
          <span>🖋</span> Гравировка
          {engravingList.length > 0 && <span className="bg-white/30 px-1.5 rounded-full text-[10px]">{engravingList.length}</span>}
        </button>
        ...
        <div className="mb-8">
          <div className="bg-slate-900 rounded-[20px] p-6 mb-6 flex flex-col md:flex-row justify-between items-center shadow-lg">
            ...
            <div className="mt-4 md:mt-0 text-right">
              ...
              <span className="text-2xl font-black text-blue-400">{fmt(totalKirill || 0)}</span>
            </div>
          </div>
      ...
      {/* ========== ДОБАВЛЕНО 2026-06-28: ВКЛАДКА «ГРАВИРОВКА» ========== */}
      {/* Источник: repairs[].engraving (enabled=true). Отображается как отдельная карточка/единица статистики. */}
      {view === "engraving" && (
        <div className="mb-8">
          {/* Шапка-сводка */}
          <div className="bg-gradient-to-br from-purple-700 via-purple-800 to-purple-900 rounded-[20px] p-6 mb-6 flex flex-col md:flex-row justify-between items-center shadow-xl">
            <div>
              <h3 className="text-lg font-bold text-white tracking-tight flex items-center gap-2">🖋 Гравировка — отдельная единица</h3>
              <p className="text-xs text-purple-200 mt-1">Источник: блок «Гравировка» в ремонтах (вкл. переключателем)</p>
            </div>
            <div className="mt-4 md:mt-0 flex gap-6 text-right">
              <div>
                <span className="block text-[10px] font-bold text-purple-200 uppercase tracking-wider">Работ</span>
                <span className="text-2xl font-black text-white">{engravingTotals.count}</span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-purple-200 uppercase tracking-wider">Клиенту</span>
                <span className="text-2xl font-black text-white">{fmt(engravingTotals.price)}</span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-blue-200 uppercase tracking-wider">Кириллу</span>
                <span className="text-2xl font-black text-blue-300">{fmt(engravingTotals.kirill)}</span>
              </div>
              <div>
                <span className="block text-[10px] font-bold text-emerald-200 uppercase tracking-wider">Наценка</span>
                <span className="text-2xl font-black text-emerald-300">{fmt(engravingTotals.markup)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {engravingList.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-100">
                <span className="text-4xl block mb-3">🖋</span>
                <p className="text-slate-400 font-medium">Нет ремонтов с включённой гравировкой</p>
                <p className="text-xs text-slate-300 mt-2">Откройте ремонт → включите переключатель «Гравировка» → заполните ТЗ и суммы</p>
              </div>
            ) : engravingList.map(r => (
              <div key={r.id} className="bg-white rounded-[20px] shadow-sm border border-purple-100 overflow-hidden hover:shadow-md transition-all">
                <div className="p-5 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className="w-14 h-14 rounded-xl bg-purple-50 border border-purple-100 flex items-center justify-center shrink-0 text-2xl">🖋</div>
                    <div className="min-w-0">
                      {r.orderNumber && <span className="text-[10px] font-bold text-slate-400 block mb-0.5">№ {r.orderNumber}</span>}
                      <div className="font-bold text-slate-800 text-base truncate">{r.clientName || "Без имени"}</div>
                      <div className="text-xs text-slate-500 mt-0.5 truncate">
                        {(r.items || []).map(it => it.type).filter(Boolean).slice(0, 2).join(", ") || "Ремонт"}
                      </div>
                      <div className="text-[10px] text-slate-400 mt-1 flex items-center gap-2">
                        <span>📅 {fmtDate(r.orderDate)}</span>
                        {r.pickupPoint && <span className="bg-slate-100 px-1.5 py-0.5 rounded font-bold">📍 {r.pickupPoint}</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-wrap items-stretch gap-2 text-right">
                    <div className="bg-purple-50 border border-purple-200 px-3 py-2 rounded-lg min-w-[80px]">
                      <div className="text-[9px] font-bold text-purple-600 uppercase tracking-wider">Клиенту</div>
                      <div className="text-base font-black text-purple-700">{fmt(r._engravingPrice)}</div>
                    </div>
                    <div className="bg-blue-50 border border-blue-200 px-3 py-2 rounded-lg min-w-[80px]">
                      <div className="text-[9px] font-bold text-blue-600 uppercase tracking-wider">Кириллу</div>
                      <div className="text-base font-black text-blue-700">{fmt(r._engravingKirill)}</div>
                    </div>
                    {r._engravingOutsource > 0 && (
                      <div className="bg-rose-50 border border-rose-200 px-3 py-2 rounded-lg min-w-[80px]">
                        <div className="text-[9px] font-bold text-rose-600 uppercase tracking-wider">Аутсорс</div>
                        <div className="text-base font-black text-rose-700">−{fmt(r._engravingOutsource)}</div>
                      </div>
                    )}
                    <div className="bg-emerald-50 border border-emerald-200 px-3 py-2 rounded-lg min-w-[80px]">
                      <div className="text-[9px] font-bold text-emerald-600 uppercase tracking-wider">Наценка</div>
                      <div className="text-base font-black text-emerald-700">{fmt(r._engravingMarkup)}</div>
                    </div>
                  </div>
                </div>
                {r.engraving.task && (
                  <div className="px-5 pb-4 border-t border-purple-50 pt-3">
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">ТЗ гравировщику</div>
                    <div className="bg-purple-50/50 border border-purple-100 rounded-lg p-3 text-xs text-slate-700 whitespace-pre-wrap">{r.engraving.task}</div>
                  </div>
                )}
                {r.engraving.taskImages && r.engraving.taskImages.length > 0 && (
                  <div className="px-5 pb-4">
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">📎 Фото к ТЗ ({r.engraving.taskImages.length})</div>
                    <div className="flex flex-wrap gap-2">
                      {r.engraving.taskImages.map((img, i) => (
                        <img
                          key={i}
                          src={img}
                          alt={`Гравировка ${i+1}`}
                          onClick={() => onOpenViewer && onOpenViewer(r.engraving.taskImages, i)}
                          className="w-16 h-16 object-cover rounded-lg border border-slate-200 cursor-zoom-in hover:opacity-80 hover:scale-105 transition-all"
                        />
                      ))}
                    </div>
                  </div>
