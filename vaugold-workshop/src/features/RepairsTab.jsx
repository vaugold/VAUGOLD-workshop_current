...
const RepairReceipt = ({ repair, onClose }) => {
  ...
  // ДОБАВЛЕНО 2026-06-28: + engravingPrice в итого чека (если включена)
  const engravingPriceReceipt = (repair.engraving && repair.engraving.enabled) ? (parseFloat(repair.engraving.price) || 0) : 0;
  const totalPrice = items.reduce((s, it) => s + (parseFloat(it.price) || 0) + (it.extras || []).reduce((s2, e) => s2 + (parseFloat(e.price) || 0), 0), 0) + engravingPriceReceipt;
      ...
      <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="rr-wrap">
          <div className="r-wrap">
            ...
            {/* ДОБАВЛЕНО 2026-06-28: Гравировка — единой строкой в клиентском чеке (без разбивки) */}
            {mode === "client" && repair.engraving && repair.engraving.enabled && (parseFloat(repair.engraving.price) || 0) > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontWeight: "bold", color: "#2C1F33", marginTop: 4, marginBottom: 8, paddingTop: 6, borderTop: "1px dashed #ddd" }}>
                <span>🖋 Graveerimine / Гравировка</span>
                <span>{fmt(repair.engraving.price)}</span>
              </div>
            )}
...
// Генератор пустого блока «Гравировка» (на уровне всего ремонта, не внутри items)
const emptyEngraving = () => ({
  enabled: false,
  task: "",
  taskImages: [],
  price: "",           // ОБЩАЯ стоимость в чек клиенту
  kirillCost: "",      // доля Кирилла (внутреннее)
  outsourceCost: ""    // себестоимость аутсорса (внутреннее)
});
...
const createEmptyRepair = (orderNumber = "", userRole = 'superuser') => ({
  ...
  // ДОБАВЛЕНО 2026-06-28: блок гравировки (опциональный — показывается если enabled=true)
...
export const RepairsTab = ({ repairs = [], setRepairs, allOrders = [], allCnc = [], sources = [], ensureOrderImages, onOpenViewer }) => {
  ...
  // ДОБАВЛЕНО 2026-06-29: фильтры по дате — месяц, год, период (от-до). Работают вместе (AND).
  const currentYear = new Date().getFullYear();
  const [yearFilter, setYearFilter] = useState("Все");       // "Все" | "2026" | "2025" ...
  const [monthFilter, setMonthFilter] = useState("Все");      // "Все" | "01".."12"
  const [dateFrom, setDateFrom] = useState("");              // YYYY-MM-DD
  const [dateTo, setDateTo] = useState("");                  // YYYY-MM-DD
  // ДОБАВЛЕНО 2026-06-29: фильтр по точке приёма (Все / EM / VAU)
  const [pointFilter, setPointFilter] = useState("Все");      // "Все" | "EM" | "VAU"
  // ИСПРАВЛЕНО 2026-06-29: фильтр по источнику удалён — теперь только 2 точки приёма,
  // и в ремонтах источник больше не хранится.
  // ДОБАВЛЕНО 2026-06-29: локальные черновики комментариев карточки — чтобы не дёргать
  // setRepairs (а через него — saveArray в Supabase) на каждое нажатие клавиши.
  // Коммит в repairs[] происходит при onBlur / по таймеру.
  const [commentDrafts, setCommentDrafts] = useState({}); // { [repairId]: string }
  const commentTimersRef = useRef({}); // { [repairId]: timeoutId }
  ...
  // ДОБАВЛЕНО 2026-06-28: Гравировка — опциональный блок. Если включён — его цена идёт клиенту,
  // а outsource-часть вычитается из дохода мастерской (как обычный outsource).
  const engravingEnabled = !!(form.engraving && form.engraving.enabled);
  const engravingPrice   = engravingEnabled ? (parseFloat(form.engraving.price) || 0) : 0;
  const engravingOutsource = engravingEnabled ? (parseFloat(form.engraving.outsourceCost) || 0) : 0;
  const engravingMarkup  = engravingEnabled ? Math.max(0, engravingPrice - (parseFloat(form.engraving.kirillCost) || 0) - engravingOutsource) : 0;
  const engravingKirill  = engravingEnabled ? (parseFloat(form.engraving.kirillCost) || 0) : 0;

  const totalPrice = form.items.reduce((s, it) => s + (parseFloat(it.price) || 0), 0) + extrasPrice + engravingPrice;
  // Себестоимость аутсорса: сумма outsourceCost из разбивки по мастерам + аутсорс гравировки
  ...
  const updateRepairField = (id, field, value) => {
    setRepairs(repairs.map(x => {
      ...
      // ДОБАВЛЕНО 2026-06-29: при смене статуса приёмщика на «Передано мастеру» автоматом ставим мастеру «В работе» (если пусто)
      if (field === "receptionStatus" && value === "Передано мастеру" && !updated.masterStatus) {
        updated.masterStatus = "В работе";
      }
  ...
  const filteredRepairs = useMemo(() => {
    return repairs.filter(r => {
      ...
      // ДОБАВЛЕНО 2026-06-29: вкладка «Активные» — всё, что не выдано клиенту.
      // Для L24 ещё требуется отметка «Оплачено» (им счёт выставляется раз в период).
      if (statusFilter === "🟢 Активные") {
        if (r.receptionStatus === "Выдано клиенту") return false;
        if (r.location === "L24" && r.l24PaymentStatus !== "Оплачено") return false;
        // (фильтры по дате применяются ниже)
      } else if (statusFilter === "⏳ Ожидает клиента") {
        // ДОБАВЛЕНО 2026-06-29: вкладка «Ожидает клиента»
      ...
      // === ДОБАВЛЕНО 2026-06-29: фильтр по точке приёма (EM / VAU) ===
      if (pointFilter === "EM" && r.pickupPoint !== "Sikupilli") return false;
      if (pointFilter === "VAU" && r.pickupPoint !== "Vaugold") return false;

      // === ДОБАВЛЕНО 2026-06-29: фильтры по дате (месяц / год / период от-до) ===
      if (r.orderDate) {
        ...
        // Если фильтр по дате включён, а у ремонта нет даты — скрываем
        return false;
      }
      return true;
  ...
  // ДОБАВЛЕНО 2026-06-29: счётчик «Активных» (для бейджа на кнопке)
  ...
  // ДОБАВЛЕНО 2026-06-29: счётчик «Ожидает клиента»
  const awaitingClientCount = useMemo(() => {
    return repairs.filter(r => r.awaitingClient && r.receptionStatus !== "Выдано клиенту").length;
  }, [repairs]);

  // ДОБАВЛЕНО 2026-06-29: список годов из данных (для селекта)
  const availableYears = useMemo(() => {
    const ys = new Set([String(currentYear)]);
    for (const r of repairs) {
      if (r.orderDate && /^\d{4}/.test(r.orderDate)) ys.add(r.orderDate.slice(0, 4));
    }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {view === "new" && (
        <div className="bg-white p-6 md:p-8 rounded-[24px] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-100 relative">
          ...
          <form onSubmit={e => handleSave(e, false)} className="space-y-8">
            ...
            {/* ============================================================ */}
            {/* БЛОК: 🖋 ГРАВИРОВКА (опциональный, между позициями и финансами) */}
            {/* ДОБАВЛЕНО 2026-06-28: */}
            {/*   - В чек клиента идёт ТОЛЬКО общая сумма (engravingPrice) */}
            {/*   - Разбивка (Кирилл / Аутсорс / Наценка) — внутренняя статистика */}
            {/* ============================================================ */}
            <div className="bg-gradient-to-br from-purple-50/80 to-white p-5 rounded-2xl border border-purple-200/60 space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-black text-purple-900 uppercase tracking-wider flex items-center gap-2">
                  <span>🖋</span> Гравировка
                </h3>
                <label className="flex items-center gap-2 cursor-pointer bg-white border border-purple-200 px-3 py-1.5 rounded-lg hover:bg-purple-50 transition-colors">
                  <input
                    type="checkbox"
                    checked={engravingEnabled}
                    onChange={e => set("engraving", { ...(form.engraving || emptyEngraving()), enabled: e.target.checked })}
                    className="w-4 h-4 rounded text-purple-600 cursor-pointer"
                  />
                  <span className="text-xs font-bold text-purple-900">{engravingEnabled ? "Включено" : "Выключено"}</span>
                </label>
              </div>

              {engravingEnabled && (
                <>
                  {/* === ОБЩАЯ СТОИМОСТЬ — НЕМНОГО БОЛЬШЕ ОБЫЧНОГО, НА ВСЮ ШИРИНУ === */}
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Стоимость (в чек)</label>
                    <input
                      type="number"
                      min="0"
                      placeholder="0"
                      className="w-full px-3 py-2 bg-purple-50/70 border border-purple-200 rounded-lg text-xl font-black text-slate-900 outline-none focus:border-purple-400 tabular-nums"
                      value={form.engraving?.price ?? ""}
                      onChange={e => set("engraving", { ...(form.engraving || emptyEngraving()), price: e.target.value })}
                    />
                  </div>

                  {/* === РАЗБИВКА — КАК ОБЫЧНЫЕ ПОЛЯ РЕМОНТА (text-sm font-bold) === */}
                  <div className="border-t border-purple-200/60 pt-3 space-y-2">
                    {/* Kirill (работа) */}
                    <div className="flex items-center gap-3">
                      <label className="text-[10px] font-bold text-slate-400 uppercase mb-0 block w-32 shrink-0">Kirill (работа)</label>
                      <input
                        type="number"
                        min="0"
                        placeholder="0"
                        className="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-blue-700 outline-none focus:border-blue-400 tabular-nums"
                        value={form.engraving?.kirillCost ?? ""}
                        onChange={e => set("engraving", { ...(form.engraving || emptyEngraving()), kirillCost: e.target.value })}
                      />
                    </div>

                    {/* Outsource (себест.) */}
                    <div className="flex items-center gap-3">
                      <label className="text-[10px] font-bold text-slate-400 uppercase mb-0 block w-32 shrink-0">Outsource</label>
                      <input
                        type="number"
                        min="0"
                        placeholder="0"
                        className="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-rose-700 outline-none focus:border-rose-400 tabular-nums"
                        value={form.engraving?.outsourceCost ?? ""}
                        onChange={e => set("engraving", { ...(form.engraving || emptyEngraving()), outsourceCost: e.target.value })}
                      />
                    </div>

                    {/* Наценка (авто-расчёт) */}
                    <div className="flex items-center gap-3">
                      <label className="text-[10px] font-bold text-slate-400 uppercase mb-0 block w-32 shrink-0">Наценка</label>
                      <div className="flex-1 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-amber-700 tabular-nums">
                        {fmt(engravingMarkup)}
                      </div>
                    </div>
                  </div>

                  {/* === ТЗ + ФОТО — ВНИЗУ (компактно) === */}
                  <div className="border-t border-purple-200/60 pt-3 space-y-2">
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">ТЗ для гравировщика</label>
                      <textarea
                        rows={2}
                        placeholder="Что нужно выгравировать (текст, шрифт, глубина и т.п.)"
                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm outline-none resize-none focus:border-purple-400"
                        value={form.engraving?.task || ""}
                        onChange={e => set("engraving", { ...(form.engraving || emptyEngraving()), task: e.target.value })}
                        onPaste={e => {
                          const items = [...e.clipboardData.items];
                          const imgItem = items.find(it => it.type.startsWith("image/"));
                          if (imgItem) {
                            ...
                            const f = imgItem.getAsFile();
                            const r = new FileReader();
                            r.onload = ev => set("engraving", { ...(form.engraving || emptyEngraving()), taskImages: [...((form.engraving && form.engraving.taskImages) || []), ev.target.result] });
                            r.readAsDataURL(f);
                          }
                        }}
                      />
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">📎 Фото к ТЗ</label>
                      <div className="flex flex-wrap gap-2">
                        {((form.engraving && form.engraving.taskImages) || []).map((img, ti) => (
                          <div key={ti} className="relative group">
                            <img
                              src={img}
                              alt=""
                              onClick={() => onOpenViewer && onOpenViewer((form.engraving && form.engraving.taskImages) || [], ti)}
                              className="w-14 h-14 object-cover rounded border border-slate-200 cursor-pointer"
                            />
                            <button
                              type="button"
                              onClick={() => set("engraving", { ...(form.engraving || emptyEngraving()), taskImages: ((form.engraving && form.engraving.taskImages) || []).filter((_, j) => j !== ti) })}
                              className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-rose-500 text-white text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
                            >&times;</button>
                          </div>
                        ))}
                        <label className="w-14 h-14 border-2 border-dashed border-purple-300 rounded flex flex-col items-center justify-center cursor-pointer text-purple-400 hover:border-purple-500 hover:text-purple-600 transition-colors bg-white">
                          <span className="text-lg leading-none">+</span>
                          <span className="text-[8px] font-bold">ФОТО</span>
                          <input type="file" accept="image/*" multiple className="hidden" onChange={e => {
                            Array.from(e.target.files).forEach(f => {
                              const r = new FileReader();
                              r.onload = ev => set("engraving", { ...(form.engraving || emptyEngraving()), taskImages: [...((form.engraving && form.engraving.taskImages) || []), ev.target.result] });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-slate-100 pt-6">
              ...
              <div className="bg-slate-900 text-white p-6 rounded-[24px] shadow-xl">
                <div className="flex justify-between text-sm mb-1"><span className="text-slate-400">Стоимость работ:</span><span className="font-semibold">{fmt(totalPrice - extrasPrice - engravingPrice)}</span></div>
                ...
                {engravingEnabled && (
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-purple-300 flex items-center gap-1">🖋 Гравировка:</span>
                    <span className="font-semibold text-purple-200">{fmt(engravingPrice)}</span>
                  </div>
                )}
                ...
                <div className="border-t border-slate-700 my-3 pt-3">
                  ...
                  {/* ДОБАВЛЕНО 2026-06-28: расшифровка гравировки (внутренняя) */}
                  {engravingEnabled && (
                    <div className="mt-3 mb-2 p-3 bg-slate-800/60 rounded-xl border border-slate-700/60 space-y-1.5">
                      <div className="text-[9px] font-bold text-purple-300 uppercase tracking-wider mb-1.5">🖋 Разбивка гравировки (внутр.)</div>
                      <div className="flex justify-between text-xs"><span className="text-slate-400">Кириллу (работа):</span><span className="font-semibold text-blue-300">{fmt(engravingKirill)}</span></div>
                      {engravingOutsource > 0 && (
                        <div className="flex justify-between text-xs"><span className="text-slate-400">Аутсорс (себест.):</span><span className="font-semibold text-rose-300">-{fmt(engravingOutsource)}</span></div>
                      )}
                      <div className="flex justify-between text-xs border-t border-slate-700 pt-1.5 mt-1.5"><span className="text-slate-400">Наценка мастерской:</span><span className="font-semibold text-emerald-300">{fmt(engravingMarkup)}</span></div>
                    </div>
                  )}
            ...
            {/* ДОБАВЛЕНО 2026-06-29: блок «Итоговый комментарий» (виден всем, включая мастеров).
               В карточке журнала его также можно редактировать без открытия ремонта. */}
            <div className="bg-amber-50/60 border border-amber-200 rounded-2xl p-4 space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[10px] font-bold text-amber-800 uppercase tracking-wider flex items-center gap-1.5">
                  <span>📝</span> Итоговый комментарий
                </label>
                <span className="text-[10px] text-amber-700/80 italic">виден мастеру и приёмщику</span>
              </div>
              <textarea
                rows={3}
                placeholder="Согласования с клиентом, особые пожелания, важные заметки для мастера…"
                className="w-full px-3 py-2 bg-white border border-amber-200 rounded-lg text-sm outline-none resize-none focus:border-amber-400"
                value={form.comment || ""}
      ...
      {view === "list" && (
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-3 mb-6 bg-white p-4 rounded-[20px] shadow-sm border border-slate-100">
            {/* ===================================================== */}
            {/* ПОЛОСА 1: СТАТУСЫ РЕМОНТА */}
            {/* ===================================================== */}
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mr-1">Статус:</span>
              <button onClick={() => setStatusFilter("🟢 Активные")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${statusFilter === "🟢 Активные" ? 'bg-emerald-600 text-white shadow-md' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'}`}>
                🟢 Активные ({activeRepairsCount})
              </button>
              {REPAIR_RECEPTION_STATUSES.map(s => (
              ...
              {REPAIR_MASTER_STATUSES.map(s => (
                <button key={s} onClick={() => setStatusFilter(s)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${statusFilter === s ? 'bg-slate-800 text-white shadow-md' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>
                  {s}
                </button>
              ))}
              <button onClick={() => setStatusFilter("⏳ Ожидает клиента")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${statusFilter === "⏳ Ожидает клиента" ? 'bg-violet-600 text-white shadow-md' : 'bg-violet-50 text-violet-700 hover:bg-violet-100'}`}>
                ⏳ Ожидает клиента ({awaitingClientCount})
              </button>
              ...
              <button onClick={() => setStatusFilter("Все")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${statusFilter === "Все" ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                Все ({repairs.filter(r => !r.isDraft).length})
              </button>
            </div>

            {/* ===================================================== */}
            {/* ПОЛОСА 2: ТРАНСПОРТИРОВКА (курьер) */}
            {/* ===================================================== */}
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mr-1">🛵 Курьер:</span>
              <button onClick={() => setStatusFilter("🛵 Курьер (любой)")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${statusFilter === "🛵 Курьер (любой)" ? 'bg-amber-600 text-white shadow-md' : 'bg-amber-50 text-amber-700 hover:bg-amber-100'}`}>
                Все для транспортировки ({repairs.filter(r => (r.courierVauSiku || r.courierSikuVau) && r.receptionStatus !== "Выдано клиенту").length})
              </button>
              <button onClick={() => setStatusFilter("🛵 VAU→Sikupilli")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${statusFilter === "🛵 VAU→Sikupilli" ? 'bg-orange-600 text-white shadow-md' : 'bg-orange-50 text-orange-700 hover:bg-orange-100'}`}>
                VAU → Sikupilli ({repairs.filter(r => r.courierVauSiku && r.receptionStatus !== "Выдано клиенту").length})
              </button>
              <button onClick={() => setStatusFilter("🛵 Sikupilli→VAU")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${statusFilter === "🛵 Sikupilli→VAU" ? 'bg-teal-600 text-white shadow-md' : 'bg-teal-50 text-teal-700 hover:bg-teal-100'}`}>
                Sikupilli → VAU ({repairs.filter(r => r.courierSikuVau && r.receptionStatus !== "Выдано клиенту").length})
              </button>
            </div>

            {/* ===================================================== */}
            {/* ПОЛОСА 3: ТОЧКА ПРИЁМА */}
            {/* ===================================================== */}
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mr-1">📍 Точка приёма:</span>
              <button onClick={() => setPointFilter("Все")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${pointFilter === "Все" ? 'bg-slate-800 text-white shadow-md' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>Все</button>
              <button onClick={() => setPointFilter("EM")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${pointFilter === "EM" ? 'bg-purple-600 text-white shadow-md' : 'bg-purple-50 text-purple-700 hover:bg-purple-100'}`}>🏪 EM ({repairs.filter(r => r.pickupPoint === 'Sikupilli' && !r.isDraft).length})</button>
              <button onClick={() => setPointFilter("VAU")} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${pointFilter === "VAU" ? 'bg-indigo-600 text-white shadow-md' : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'}`}>💎 VAU ({repairs.filter(r => r.pickupPoint === 'Vaugold' && !r.isDraft).length})</button>
            </div>

            {/* ===================================================== */}
            {/* ПОЛОСА 4: ДАТЫ (месяц/год/период) */}
            {/* ===================================================== */}
            <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-slate-100">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">📅 Дата:</span>
              <select
                value={monthFilter}
                onChange={e => setMonthFilter(e.target.value)}
                className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none cursor-pointer hover:bg-slate-100"
              >
                <option value="Все">Месяц: все</option>
                {["01","02","03","04","05","06","07","08","09","10","11","12"].map(m => (
                  <option key={m} value={m}>{new Date(2000, parseInt(m)-1, 1).toLocaleString("ru", { month: "long" })}</option>
                ))}
              </select>
              <select
                value={yearFilter}
                ...
                className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none cursor-pointer hover:bg-slate-100"
              >
                <option value="Все">Год: все</option>
                {availableYears.map(y => (
                  <option key={y} value={y}>{y}</option>
                ))}
              </select>
              <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-2 py-1">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Период</span>
                <input
                  type="date"
                  value={dateFrom}
                  onChange={e => setDateFrom(e.target.value)}
                  className="bg-transparent outline-none text-xs font-semibold text-slate-700 w-[120px]"
                />
              ...
              {(yearFilter !== "Все" || monthFilter !== "Все" || dateFrom || dateTo || pointFilter !== "Все") && (
                <button
                  onClick={() => { setYearFilter("Все"); setMonthFilter("Все"); setDateFrom(""); setDateTo(""); setPointFilter("Все"); }}
                  className="px-3 py-2 text-xs font-bold text-rose-600 bg-rose-50 border border-rose-200 rounded-xl hover:bg-rose-100 transition-colors"
                >
                  ✕ Сбросить всё
                </button>
              )}
                  ...
                  <div className="p-5 cursor-pointer flex flex-col md:flex-row gap-4 items-start md:items-center justify-between" onClick={() => {
                    ...
                    <div className="flex flex-col items-stretch md:items-end gap-2 text-right w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-none border-slate-100" onClick={e => e.stopPropagation()}>
                      ...
                      <div className="flex flex-col gap-1.5 mt-1 bg-slate-50 p-2 rounded-lg border border-slate-200">
                        ...
                        {/* ДОБАВЛЕНО 2026-06-29: активный статус приёмщика прямо в карточке */}
                        <select
                          className={`text-[10px] font-semibold px-2 py-1 rounded border outline-none cursor-pointer ${r.receptionStatus === "Выдано клиенту" ? "bg-slate-100 border-slate-300 text-slate-500" : r.receptionStatus === "Передано мастеру" ? "bg-emerald-100 border-emerald-300 text-emerald-800" : "bg-white border-slate-200 text-slate-600"}`}
                          value={r.receptionStatus || ""}
                          onChange={e => updateRepairField(r.id, "receptionStatus", e.target.value)}
                        >
                          {REPAIR_RECEPTION_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                        <select
                    ...
                    <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex flex-col md:flex-row gap-6 animate-fade-in">
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div className="space-y-3">
                          ...
                          {/* ДОБАВЛЕНО 2026-06-29: Итоговый комментарий — редактируемый прямо в карточке (без открытия формы).
                               Используем локальный черновик + debounce, чтобы при вводе не триггерить
                               saveArray в Supabase на каждое нажатие (это ломало ввод). */}
                          <div className="bg-amber-50/60 border border-amber-200 rounded-xl p-3 space-y-1.5" onClick={e => e.stopPropagation()}>
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-bold text-amber-800 uppercase tracking-wider flex items-center gap-1.5">
                                <span>📝</span> Итоговый комментарий
                              </span>
                              <span className="text-[9px] text-amber-700/80 italic">можно править тут</span>
                            </div>
                            <textarea
                              rows={3}
                              placeholder="Заметки, согласования, важное для мастера…"
                              className="w-full px-2.5 py-1.5 bg-white border border-amber-200 rounded-lg text-xs outline-none resize-none focus:border-amber-400 text-slate-700"
                              value={commentDrafts[r.id] !== undefined ? commentDrafts[r.id] : (r.comment || "")}
                              onChange={e => {
                                const v = e.target.value;
                                setCommentDrafts(p => ({ ...p, [r.id]: v }));
                                // Debounce коммита: через 600мс после последнего нажатия
                                if (commentTimersRef.current[r.id]) clearTimeout(commentTimersRef.current[r.id]);
                                commentTimersRef.current[r.id] = setTimeout(() => {
                                  updateRepairField(r.id, "comment", v);
                                  setCommentDrafts(p => {
                                    const n = { ...p };
                                    delete n[r.id];
                                    return n;
                              ...
                              onBlur={e => {
                                // При потере фокуса — коммитим сразу (если есть draft)
                                if (commentTimersRef.current[r.id]) clearTimeout(commentTimersRef.current[r.id]);
                                const draft = commentDrafts[r.id];
                                if (draft !== undefined && draft !== (r.comment || "")) {
                                  updateRepairField(r.id, "comment", draft);
                                }
                                setCommentDrafts(p => {
                                  const n = { ...p };
                                  delete n[r.id];
