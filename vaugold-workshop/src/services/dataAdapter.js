...
// ИСПРАВЛЕНО 2026-06-29: убрали слепое добавление created_at/updated_at.
// Некоторые таблицы (cnc_clients, cnc_items, sources) этих колонок не имеют,
// а Postgres 400-ит весь SELECT. Теперь добавляем только если таблица
// про них знает (запоминаем в кэше на уровне модуля).
const OPTIONAL_COLS = ['created_at', 'updated_at'];
const _hasColumnCache = {}; // key -> true (есть) | false (нету)

const tableHasCols = async (table) => {
  if (_hasColumnCache[table] !== undefined) return _hasColumnCache[table];
  // Запрашиваем минимальный запрос с этими колонками
  try {
    const { error } = await supabase.from(table).select(`id, ${OPTIONAL_COLS.join(', ')}`).limit(1);
    if (!error) {
      _hasColumnCache[table] = true;
      return true;
    }
    _hasColumnCache[table] = false;
    return false;
  } catch {
    _hasColumnCache[table] = false;
    return false;
...
const buildSelectList = (cfg) => {
  ...
  // Не добавляем created_at/updated_at — это опциональные и проверяются в tableHasCols()
  return Array.from(cols).join(', ');
};

// Асинхронная версия — выбирает только те колонки, которые реально есть в таблице.
// Используется в loadArray() — там мы можем позволить себе лишний round-trip для retry.
const buildSelectListAsync = async (cfg, table) => {
  if (!cfg.fields) return '*';
  const cols = new Set(['id']);
  for (const snake of Object.values(cfg.fields)) cols.add(snake);
  if (cfg.fieldAliases) {
  ...
  if (await tableHasCols(table)) {
    for (const c of OPTIONAL_COLS) cols.add(c);
  }
  return Array.from(cols).join(', ');
};
...
export const loadArray = async (key) => {
  ...
  // 1. Достаём строки из новой таблицы (ИСПРАВЛЕНО 2026-06-29: сначала без created_at/updated_at,
  //    потом докидываем их если таблица их поддерживает — иначе Postgres 400-ит весь SELECT)
  let rows = null;
  let rowsError = null;
  try {
    const list = await buildSelectListAsync(cfg, cfg.table);
    const r = await supabase.from(cfg.table).select(list);
    rows = r.data; rowsError = r.error;
  } catch (e) {
    rowsError = e;
  }
...
export const saveArray = async (key, newArr) => {
  ...
  if (cfg.arrayItemKey && Object.keys(cfg.fields).length === 1 && cfg.fields[cfg.arrayItemKey]) {
    ...
    // SELECT все текущие (ИСПРАВЛЕНО 2026-06-29: is_active — опциональная колонка, проверяем как в buildSelectListAsync)
    const r1 = await supabase.from(cfg.table).select(`id, ${cfg.arrayItemKey}`);
    if (r1.error) throw r1.error;
    const baseRows = r1.data || [];
    let currentRows = baseRows;
    if (cfg.softDelete) {
      const r2 = await supabase.from(cfg.table).select(`id, is_active`);
      if (!r2.error && r2.data) {
        const isActiveMap = new Map(r2.data.map(r => [r.id, r.is_active]));
        currentRows = baseRows.map(r => ({ ...r, is_active: isActiveMap.get(r.id) }));
      }
    }
